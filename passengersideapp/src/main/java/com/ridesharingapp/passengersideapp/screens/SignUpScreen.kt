...
import com.ridesharingapp.passengersideapp.data.registration.SignUpUIEvent
...
fun SignUpScreen(signUpViewModel: SignUpViewModel) {
    ...
    if (uiState.displayUserCollisionMsg) {
        AlertDialog(
            onDismissRequest = { signUpViewModel.dismissUserCollisionMsg() },
            confirmButton = {
                TextButton(onClick = { signUpViewModel.dismissUserCollisionMsg() }) {
                    Text(text = "OK")
                }
            },
            text = {
                Text("You have already has an account associated with this email address." +
